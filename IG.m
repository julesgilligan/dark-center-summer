function [IGB]=IG(apx1,apy1,parity,p,m,e,w0,k,z,X,Y,px)
%{
...
%% INCE GAUSSIAN BEAM
if z==0 % At the z=0
   [xhi,etha]=mesh_elliptic(f0,apx1,apy1,X,Y,px); % Calculate elliptic coordinates (xhi,etha), (X,Y)= Cartesian Coordinates
...
function [xi,eta]=mesh_elliptic(f,apx1,apy1,X,Y,px)
%{
Creates an elliptic coordinate mesh
...
N=((2*apy1)/px)+1;
P=((2*apx1)/px)+1;
xi=zeros(N,P); eta=zeros(N,P);  % Initialization

% Calculate First Quadrant   


en = acosh((X(1:(N+1)/2,(P+1)/2:P)+1i*Y(1:(N+1)/2,(P+1)/2:P))/f);
ee = real(en);
nn = imag(en);
nn = nn + (nn<0)*2*pi; 
xi(1:(N+1)/2,(P+1)/2:P)=ee;
eta(1:(N+1)/2,(P+1)/2:P)=nn;
      
% Calculate other quadrants by symmetry       
xi(1:(N+1)/2 , 1:(P-1)/2)=fliplr(xi(1:(N+1)/2,(P+3)/2:P));
xi((N+3)/2:N,1:P)=flipud(xi(1:(N-1)/2,1:P));
eta(1:(N+1)/2,1:(P-1)/2)=pi-fliplr(eta(1:(N+1)/2,(P+3)/2:P));
eta((N+3)/2:N,1:P)=pi+rot90((eta(1:(N-1)/2,1:P)),2);
